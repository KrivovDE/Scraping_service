...
__all__ = ('hh_vrn_py', 'sj_vrn_py', 'ha_vrn_py')
...
def hh_vrn_py(url, city=None, language=None):
    jobs = []
    errors = []
    if url:
...
def sj_vrn_py(url, city=None, language=None):
    jobs = []
    errors = []
...
def ha_vrn_py(url, city=None, language=None):
    jobs = []
    errors = []
    ...
    if url:
        ...
        if resp.status_code == 200:
            ...
            if main_div:
                div_lst = main_div.find_all('div', attrs={'class': 'section-box'})
                for div in div_lst:
                    title = div.find('span', attrs={'class': 'vacancy-card__title'})
                    href = div.find('span', attrs={'class': 'vacancy-card__title'})
                    cont = div.find('div', attrs={'class': 'vacancy-card__meta'})
                    content = cont.text
                    company = 'No name'
